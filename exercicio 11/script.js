...
function addPlayer() {
  ...
  const confirmation = confirm(
    `deseja confirmar o jogador ? ${position}, ${nome}, ${number}`
  );
  if (confirmation) {
    const teamList = document.querySelector("#team-list");
    const li = document.createElement("li");
    li.setAttribute("id", "player-" + number);
    li.textContent = `Posição: ${position}, Nome: ${nome}, Numero: ${number} `;
...
function removePlayer() {
  const numberToRemove = document.getElementById("numberRemove").value;
  const playerToRemove = document.getElementById("player-" + numberToRemove);
  const confirma = confirm(`Deseja remover jogador ? ${numberToRemove},`);
  if (confirma) {
    document.querySelector("#team-list").removeChild(playerToRemove);
    document.querySelector("#numberRemove").value = "";
